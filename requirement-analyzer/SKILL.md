...
| `--from-snapshot` | 复用已有的 `_raw/` 快照目录（格式 `yyMMdd-uuid`），跳过 Step 2-3 |
| `--output` | 指定输出目录（默认 `docs/requirement-analyzer/<yyMMdd>-<uuid8>/`） |
...
- 默认输出：`docs/requirement-analyzer/<yyMMdd>-<uuid8>/`
  - `yyMMdd` = 当前日期
  - `uuid8` = 8 位随机字符串（字母数字）
...
```
docs/requirement-analyzer/<yyMMdd>-<uuid8>/
├── SUMMARY.md                  # 需求总览汇总
├── _raw/                       # 原始快照（审计追溯）
│   ├── story_<id>.json
│   └── wiki_<token>.md
...
- `uuid8`：8 位随机标识符，避免同一天多次执行的冲突
- `<slug>`：从需求标题提取的英文短标识（如 `supplier-audit-feishu-scope`）
- 序号 `01`、`02` 在迭代内编号（两位数字，零填充）
...
```yaml
session:
  session_id: "<yyMMdd>-<uuid8>"
  output_dir: "docs/requirement-analyzer/<yyMMdd>-<uuid8>"
